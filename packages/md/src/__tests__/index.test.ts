...
describe('Md', () => {
  ...
  test('read', () => {
    expect(md.read({ filename: join(__dirname, 'h1.md') })).toEqual({
      body: '# H1\n',
    })
  })
  ...
  test('write', () => {
    ...
    expect(readFileSync(join(__dirname, `${tmp}.tmp.md`)).toString()).toEqual(
      tmp
    )
...
`,
  ...
  test('searchableContent', () => {
    ...
    expect(
      md.searchableContent({ filename: join(__dirname, `${tmp}.tmp.md`) })
    ).toEqual(`# ${tmp}.tmp\n\n`)
