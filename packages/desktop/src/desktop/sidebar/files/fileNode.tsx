...
export function FileNode({
  ...
  const { setOpens, current, setCurrent, loadAllItems, source } =
  ...
  return (
    <div className={`node file ${item.path === current ? 'current' : ''}`}>
      {mode === 'view' && (
        <div
          className='view'
      ...
      {mode === 'rename' && <RenameMode item={item} setMode={setMode} />}
      {mode === 'delete' && (
        <div className='delete'>
          <div className='message'>
            Sure to delete <span>{name}</span>?
          </div>
          <div className='button cancel-button' onClick={() => setMode('view')}>
            <XIcon />
          </div>
          <div
